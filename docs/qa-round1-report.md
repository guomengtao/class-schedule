...
- [一、本轮结论摘要](#一本轮结论摘要)
- [二、缺陷清单](#二缺陷清单)
  - [P0 — 用户操作直接受影响的功能 Bug](#p0--用户操作直接受影响的功能-bug最高优先级)
  - [P1 — 界面与显示 / 机型适配](#p1--界面与显示--机型适配)
  - [P2 — 输入与交互](#p2--输入与交互)
  - [P3 — 代码质量与稳定性](#p3--代码质量与稳定性)
  - [P4 — 安全与合规](#p4--安全与合规)
- [三、9 Pro vs 10 Pro 机型适配对比表](#三9-pro-vs-10-pro-机型适配对比表)
- [四、历史已知 Bug 修复状态核对](#四历史已知-bug-修复状态核对)
- [五、回归测试 Checklist](#五回归测试-checklist给开发可直接执行)
- [六、本轮统计](#六本轮统计)
...
|---|---|---|---|---|---|---|---|---|
| P0-1 | schedule-manager | **删除课表导致课程数据串表（致命）** | 新建 3 张课表 A/B/C → 分别录入不同课程 → 删除中间的 B | B 数据清除，C 的索引前移且数据仍对应 C | ✅ **已修复**：`deleteSchedule` 在 splice 前调用 `database.deleteScheduleAndShift` 迁移课程数据 | **致命** | `schedule-manager.ux:389-435` | `database.deleteScheduleAndShift` 已在 splice 前调用 |
| P0-2 | add-course | **新增/编辑课程保存后首页不显示** | 首页 → 新增课程 → 填写 → 保存 → 返回首页 | 首页立即出现新课程 | ✅ **已修复**：改用 `database.insertCourse` 统一写入通道，自动失效缓存 | **严重** | `add-course.ux:510` | 引入 database 模块，替换直写 storage 为 `database.insertCourse` |
...
| P0-6 | schedule-manager | **复制课表失败时卡在中间态** | 复制一张含课程的课表，过程中任一 insert 失败 | 提示完成或失败 | ✅ **已修复**：改用 `processed` 计数，无论成功失败都进收尾逻辑 | 一般 | `schedule-manager.ux:312-337`、`:328-336` | 完成判定改为"已处理数==总数"，无论成败都进收尾（刷新+还原+提示） |
...
| 编号 | 页面/组件 | 缺陷标题 | 期望 vs 实际 | 影响机型 | 严重度 | 定位 | 修复建议 |
|---|---|---|---|---|---|---|---|
| P2-1 | nickname-edit + chinese-input | **取消编辑后昵称被清空（共享键污染）** | 期望：取消不改动原值；实际：取消时写 `""` 到 `chinese_input_result` | ✅ **已修复（三重防护）**：① `chinese-input.ux` 取消时不再写 `""`，改为 `delete` 该键；② `nickname-edit.ux` 读取时加 `data !== ""` 判空；③ `chinese-input-full.ux` 同理取消不写 | **高** | `chinese-input.ux:353-369`；`nickname-edit.ux:70-82` | 已应用全部三项防护 |
| P2-2 | chinese-input-full | **方屏机器被强制走圆屏输入法布局** | 期望：按 rect/pill 布局；实际：`screentype="circle"` **写死**（已核实）→ 圆屏 480px 布局被直接缩放到方屏，按键错位 | ✅ **已修复**：通过 `device.getInfo` 动态获取 `screenShape` 传入组件 | 中 | `chinese-input-full.ux:19` | 由 `device.getInfo` 动态传入 screentype |
| P2-3 | InputMethod | **中文整词提交可越过 maxlength** | 昵称 maxlen=5，候选词整体追加（如"我们"）不触发长度校验 | ✅ **已修复**：`addAllTxt` 中增加 `maxlength` 截断逻辑 | 中 | `InputMethod.ux:471-473` | 在 `addAllTxt` 中截断到 maxlength |
...
**下一轮巡检**会重点复核：本轮列出的 P0-1、P0-2、P1-1 是否已被修复，以及是否有新增提交引入新的数据链路问题。
...
| 编号 | 缺陷 | 文件 | 改动 |
|---|---|---|---|
| ✅ P0-1 | 删除课表课程串表 | `schedule-manager.ux:389-444` | splice 前调用 `database.deleteScheduleAndShift` 迁移课程数据 |
| ✅ P0-2 | 新增课程首页不刷新 | `add-course.ux:496-515` | 用 `database.insertCourse` 替代直写 storage，自动失效缓存 |
| ✅ P0-6 | 复制课表卡中间态 | `schedule-manager.ux:308-340` | `inserted` 改为 `processed`，无论成败都进收尾 |
| ✅ P2-1 | 取消编辑清空昵称 | `chinese-input.ux:353-381` | 取消时 `delete` 键而非写 `""`；昵称页加 `data !== ""` 判空 |
| ✅ P2-2 | 方屏走圆屏键盘布局 | `chinese-input-full.ux:19/82-97` | 通过 `device.getInfo` 动态传入 `screenType` |
| ✅ P2-3 | 中文整词越过 maxlength | `InputMethod.ux:471-476` | `addAllTxt` 中按 `maxlength` 截断 |
| ✅ P2-6 | `adjustScreenWidth` 无 fail 回调 | — | 由 `chinese-input-full.ux` 的 `device.getInfo` fail 分支兜底 |

> **待修复（未触及）**：P0-3（detail 备注丢失）、P0-4（day 竞态）、P0-5（backup-restore 主题键名）、P0-7（激活持久化失败提示）、P1-1~P1-5（首页适配全系列）