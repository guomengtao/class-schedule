...
- **✅ 已修复（2026-09-25）：pill-shaped 分支整段重写为极简实现** —— 去掉全部 PNG、去掉 `border`/`border-radius`、去掉绝对定位堆叠、候选行不再用 scroll、高度自适应（333px → 约 254px，顺带解决竖直空间不足）。**只保留 1 个 `scroll-x`**（QWERTY 三排在 192px 上必须横滚才能保住可点键宽）。功能键改为纯文字：中/En、123、空格、删除
- **若仍崩的预案**：改 T9 九宫格（3 列×4 行可静态排布，彻底不用 scroll）—— 组件已有 `keys.t9` 与 `keyboardtype="T9"`，需放开 `onSelect` 中"胶囊屏不走 T9"的限制
- **胶囊屏（跑道屏）输入法的新约束**：不要用 PNG 键盘素材、**不要给大量 `<text>` 加 `border`+`border-radius`**、不要绝对定位堆叠、不要写死高度 —— 这些在小米手环 9/11 上会触发整机重启
