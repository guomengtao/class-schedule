...
- **第二轮已独立成文件**：`src/pages/input-crash-diag2/input-crash-diag2.ux`（工具 → 输入法诊断2·元素级），跑 **A~F 六项元素级隔离**（纯文本×26 / 圆角描边×26 / 横向 scroll / PNG×8 / 绝对定位堆叠 / 完整 pill 键盘）；storage key 用 `input_diag2_*` 与第一轮隔离；二维码 `dg` 用字母表达，如 `dg=C.AB`（崩在 C，A、B 通过）
- 工具栏目现有两个诊断入口：`输入法崩溃诊断`（存储/词典，第一轮）与 `输入法诊断2·元素级`（A→F）
...
- ⚠️ **极简重写已回滚（用户决定，2026-09-25，提交 `582d8c6` 已退回）**：pill 分支恢复原样（PNG / 圆角描边 / 绝对定位 / 305px 全部保留），界面不受影响。**不采用原因**：①影响所有跑道屏用户 ②键盘变小、用户不习惯。**该方案留作备选 B**（`docs/输入法崩溃第三步定位与元素级细分方案.md` 第六章）
- **当前路线 = 方案 A**：先用诊断页第二轮 A~F 定位到**具体一个元素**，再只改那一项（界面改动最小）。对应关系：B 崩→只去掉 `border`/`border-radius`（保留尺寸与 PNG）；D 崩→只换出问题的 PNG；C 崩→候选行改固定横排去掉 1 个 scroll；E 崩→只改定位方式
- **用户偏好（重要）**：不接受"为了修 bug 一次性大改界面影响所有用户"的方案 —— 宁可多花一轮定位，也要把改动面缩到最小
- **若仍崩的预案**：改 T9 九宫格（3 列×4 行可静态排布，彻底不用 scroll）—— 组件已有 `keys.t9` 与 `keyboardtype="T9"`，需放开 `onSelect` 中"胶囊屏不走 T9"的限制
- **胶囊屏（跑道屏）输入法的排查约束（尚未定位到具体项）**：PNG 键盘素材 / 大量 `<text>` 加 `border`+`border-radius` / 绝对定位堆叠 / 写死高度 —— 在小米手环 9/11 上**疑似**触发整机重启，但**未确定是哪一项**，不要凭推断一次性全改
