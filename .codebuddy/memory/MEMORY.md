...
- **实测（诊断页第一轮）**：第 1 步并发写×5 通过、第 2 步并发读×7 通过、**第 3 步（建键盘，`dictlazy=true` 不加载词典）崩溃**
- **真凶 = pill 分支「键盘 UI 渲染本身」**。已排除：W3 并发 storage I/O、W4 词典内存 OOM、W1 单帧同步初始化、W2 arc 进度条
- **教训**：连续三轮假设（布局→阻塞→存储并发→词典内存）全错。**先做可隔离的实测二分，不要凭代码推断连续猜方向**
- **当前路线 = 方案 A**：用 `input-crash-diag2`（工具 → 输入法诊断2·元素级）的 A~F 定位到**具体一个元素**再只改那一项。B 崩→只去 `border`/`border-radius`；D 崩→只换 PNG；C 崩→候选行改固定横排；E 崩→只改定位方式
- ⚠️ **极简重写已回滚（用户决定，提交 `582d8c6` 已退回）**：pill 分支恢复原样。留作备选 B（`docs/输入法崩溃第三步定位与元素级细分方案.md` 第六章）
- **若仍崩的预案**：改 T9 九宫格（3 列×4 行可静态排布，不用 scroll）—— 组件已有 `keys.t9` 与 `keyboardtype="T9"`，需放开 `onSelect` 中"胶囊屏不走 T9"的限制
